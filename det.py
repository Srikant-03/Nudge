...
prev_drowsy_state = False  # Track if drowsy score was previously above threshold
drowsy_time = 0
...
while True:
    ...
    # Check thresholds for alarm and update metrics
    if drowsy_score >= score_threshold:
        if not prev_drowsy_state:  # Count only when crossing the threshold
            drowsy_alarm_count += 1
            play_alarm()  # Play alarm only when crossing threshold
        prev_drowsy_state = True
        drowsy_time += 1  # Assuming 1 second per frame
    else:
        if prev_drowsy_state:
            stop_alarm()  # Stop alarm when below threshold
