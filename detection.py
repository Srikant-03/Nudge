...
decay_rate = 5  # How fast the score decays when not drowsy
increment_rate = 5  # How much the score increases per detection

# Bounding box area thresholds for close/far handling
MIN_BOX_AREA = 20    # Minimum bounding box area for valid detection
MAX_BOX_AREA = 100000000000000000  # Maximum bounding box area for valid detection
...
while True:
    ...
    results = model.predict(source=frame, imgsz=640, conf=0.20, device='cpu', save=False, save_txt=False)

    # Annotate frame with detection results
    drowsy_detected_now = False  # Flag for this frame's drowsy detection
    person_detected_now = False  # Flag for this frame's person detection
    ...
    for result in results:
        for box in result.boxes:
            ...
            # Check if the bounding box is within the valid size range
            if MIN_BOX_AREA <= box_area <= MAX_BOX_AREA:
                ...
                # Check for "drowsy" and "person" detections
                if class_labels[cls] == 'drowsy' and conf > 0.4:
                    drowsy_detected_now = True
                elif class_labels[cls] == 'person' and conf > 0.5:
                    person_detected_now = True

    # Update the drowsy score
    if drowsy_detected_now:
        # Increase the score significantly if "drowsy" is detected
        drowsy_score += increment_rate
    else:
        # Decrease the score (but keep it non-negative)
